...
### Creating a string for the URL
```{r}
URL_ids <- "https://api.census.gov/data/2023/acs/acs1/pums?get=SEX,PWGTP,MAR&SCHL=24"
```

### Loading the data from the API website 
```{r}
id_info <- httr::GET(URL_ids) #downloads the data and stored as id_info
str(id_info, max.level = 1) #structure of id_info
```

### Extracting the content portion because this is the data we will need to work with
```{r}
parsed_pums_info <- fromJSON(rawToChar(id_info$content)) #converting the raw data to JSON format
pums_info <- as_tibble(data_rows) #to turn into a tibble
pums_info #display tibble
```
...
summary_result <- summary.census(data_2022)
summary_result$numeric
summary_result$categorical
    } 
```
