...
```{r}
summary.census <- function(tibble = tibble, num_vars = c('AGEP', 'PWGTP'), cat_vars = 'AGE', ...){
    sample_mean <- sum(num_vars^2*PWGTP)/sum(PWGTP)
    sample_sd <- sqrt(sum(num_vars^2*PWGTP)/sum(PWGTP)-sample_mean^2)
}
return(list)
```
